...
(async () => {
  ...
  rec('CUSTOMERS · chart 1 is customer base development (customers on the left axis, ARR/customer on a quiet right axis); chart 2 is where ARR growth came from; both the same geometry', cu.titles.join('|') === 'Customer base development|Where MRR growth came from · cumulative since M0' && sameGeo(cu.rects) && cu.rects[0].width > 600 && (await D(pg, () => document.querySelectorAll('#lens-customers svg.ch .ax2').length)) === 3, JSON.stringify({ titles: cu.titles, rects: cu.rects }));
  rec('CUSTOMERS · right-edge values are the engine\'s: customers, ARR/customer, new customers, existing base, total net change; the split reconciles exactly to ARR − opening ARR', cu.rv.indexOf(Math.round(eng.cust).toLocaleString('en-GB')) >= 0 && cu.rv.indexOf(mrrS(eng.cumNew)) >= 0 && cu.rv.indexOf(mrrS(eng.cumEx)) >= 0 && cu.rv.indexOf(mrrS(eng.arr - eng.opening)) >= 0 && Math.abs(eng.cumNew + eng.cumEx - (eng.arr - eng.opening)) < 1e-6 && cu.rl.join('|').indexOf('new customers') >= 0 && cu.rl.join('|').indexOf('existing base') >= 0 && cu.rl.join('|').indexOf('total net change') >= 0, JSON.stringify({ rv: cu.rv, rl: cu.rl }));
  rec('CUSTOMERS · no bridges, no decomposition, no separate churn/contraction/expansion/GRR/NRR charts', !/CUSTOMER BASE · LOGOS|GROWTH = CUSTOMER GROWTH|− left · churned|MOVEMENT/.test(cu.txt) && cu.charts === 2, '');
