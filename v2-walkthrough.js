...
(async () => {
  ...
  step('Where is the company at month 36, and what do the ARR-only KPIs say?',
       ['World: ARR physics (the frozen v1.3 base). Company lens, month 36, MRR basis.'].concat(desc(t, ['MRR', 'cash', 'cash trough · M13 of 60', 'EBITA margin · R12M', 'gross margin · law'])).concat(rows(t, ['Opening', '+ new MRR', '− leakage⋈ 90%', '+ expansion⋈ 10%', 'Closing', 'gross dollar retention · R12M', 'net dollar retention · R12M', 'persistence', 'Revenue', 'Gross profit', 'EBITA', 'cohort M36 · 37 alive'])));
  ...
  step('How much of the loss is customers leaving, and how much is customers shrinking?',
       ['World: + Customers (logo retention 92%, contraction 5%). Persistence now reads derived on the Change rail; the Customers lens shows the logo ladder above the MRR ladder.'].concat(desc(t, ['MRR', 'customers', 'ARPA · ARR per customer'])).concat(rows(t, ['+ new logos', '− churn⋈ 92%', '− churned MRR', '− contraction⋈ 5%', 'logo retention · R12M', 'gross dollar retention · R12M', 'net dollar retention · R12M', 'persistence in force', 'Installed-base net · month'])).concat([grab(t, 'Customers stay more than dollars do', 170)]));
  ...
  step('How much of the revenue change is price, how much usage, how much adoption — and where does expansion stop?',
       ['World: + Monetization (platform €12,000 + usage 80% × 100 units × €100; drivers with caps). The expansion coefficient reads bypassed.'].concat([grab(t, 'platform · fixed', 60), grab(t, 'usage · variable', 60), grab(t, 'opening-base customer', 60), grab(t, 'usage ', 60)]).concat(rows(t, ['+ price⋈ 3% · 2%', '+ usage⋈ 15%', '+ adoption⋈ 10%', '− churn', '− contraction', 'of NRR, price · usage · adoption · R12M', 'expansion coefficient', 'persistence in force'])));
  ...
  step('When does the cash arrive, and how far is cash FCF from EBITA?',
       ['World: + Cash (billed 12 months in advance, collected one month after invoice). The waterfall continues below EBITA.'].concat(rows(t, ['EBITA', '+ Δ deferred⋈ 12 mo advance', '− Δ receivables⋈ +1 mo', 'Cash FCF'])).concat(desc(t, ['cash · M36', 'deferred revenue · M36', 'receivables · M36', 'cash FCF ÷ EBITA · R12M', 'cash trough · M1', 'capital drawn from opening cash'])));
  ...
  step('What does a retention programme cost, when does it work, and what is it worth?',
       ['World: + Hypothesis (every layer on; logo retention × 1.03 decided M6, in force M9–M32, €200k + €50k/month).'].concat([grab(t, 'hypothesis ret in force', 120)]).concat(rows(t, ['− hypothesis cost', 'EBITA', 'Cash FCF'])));
  ...
  step('And back to the frozen ARR world — is every layer off again?',
       ['Pack: ARR physics. Mechanisms: ' + JSON.stringify(mech)].concat(desc(t, ['cash'])).concat(rows(t, ['net dollar retention · R12M'])));
