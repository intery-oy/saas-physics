...
;(function (root, factory) {
  ...
  /* One scheme for the four layers: CU/MO/CA/IV for the module, CU_ON/MO_ON/
     CA_ON/IV_ON for whether it is on. (The mechanisms the run REPORTS keep
     their published names -- they are in the frozen baselines.) */
  ...
  var DEFAULT_ASSUMPTIONS = {
    ...
    /* v2 Gate C — CA_ON PHYSICS, at its NULL setting (cash moves with EBITA).
  ...
  function run(assumptions, start, horizon) {
    ...
    var IV_ON = IV.enabled(a);
    function lawAt(t) { return IV_ON ? IV.resolve(a, t) : { law: a, active: [], changes: [] }; }
    ...
    var CU_ON = CU.enabled(a);
    var cuR = CU_ON ? CU.rates(a) : null;   /* named apart from the loop scalars below */
    var P = CU_ON ? cuR.impliedPersistenceAnnual : a.persistenceAnnual;
    var aEff = CU_ON ? Object.assign({}, a, { persistenceAnnual: P }) : a;
    ...
    var MO_ON = MO.enabled(a);
    var moSpec = MO_ON ? a.monetization : null;
    var moR = MO_ON ? MO.rates(moSpec) : null;
    ...
    var moInitial = MO_ON ? stampKinds(MO.initialState(moSpec)) : null;
    var moNewLogoRevenue = MO_ON ? MO.revenue(moInitial, moR).total : null;

    /* v2 Gate C — CA_ON PHYSICS. When on, each cohort carries billing units;
    ...
    var CA_ON = CA.enabled(a);
    var BT = CA_ON ? a.billingTermMonths : null, BTIM = a.billingTiming, CDL = CA_ON ? a.collectionDelayMonths : 0;
    var arQueue = [];

    var gM = MO_ON ? null : toMonthlyPersistence(P);
    var eM = MO_ON ? null : toMonthlyExpansion(a.expansionCoefficientAnnual);
    ...
    if (CU_ON) {
      ...
      if (MO_ON) { newLogoARPA = moNewLogoRevenue; newLogoARPASource = 'monetization: per-customer components'; }
    ...
    var cohorts = seed.map(function (c, i) {
      return {
        ...
        /* v2 Gate A — customer state; null = Customer Physics off */
        initialCustomers: CU_ON ? c.customers : null,
        customers: CU_ON ? c.customers : null,
        /* v2 Gate B — per-customer component state; null = Monetization off */
        money: MO_ON ? MO.copyState(moInitial) : null,
        ...
        billingUnits: CA_ON ? CA.unitsFor(BT, BTIM, c.arr / 12, true) : null,
        rows: []
      };
    });
    var openingDeferred = CA_ON ? cohorts.reduce(function (q, c) { return q + c.billingUnits.reduce(function (w, u) { return w + u.deferred; }, 0); }, 0) : null;
    ...
    for (var t = 1; t <= H; t++) {
      ...
      var cuR_t = lawChanged && CU_ON ? CU.rates(law) : cuR;
      var bandRates_t = lawChanged ? ratesOfBands(resolveBands(CU_ON ? Object.assign({}, law, { persistenceAnnual: cuR_t.impliedPersistenceAnnual }) : law)) : bandRates;
      var moR_t = lawChanged && MO_ON ? MO.rates(law.monetization) : moR;
      var moInitial_t = lawChanged && MO_ON ? stampKinds(MO.initialState(law.monetization), moR_t) : moInitial;
      var newLogoARPA_t = !lawChanged ? newLogoARPA : (MO_ON ? MO.revenue(moInitial_t, moR_t).total : (CU_ON ? (law.newLogoARPA !== null ? law.newLogoARPA : openingARPA) : null));
      ...
      if (matured.length) {
        var nc = realiseCohort(t, matured, bandRates_t[0].name, law.grossMargin, MO_ON ? moR_t : null);
        realisedARR = nc.initialARR; realisedMRR = nc.live;
        if (CU_ON) { realisedCustomers = nc.customers; cuClose += nc.customers; }
        if (MO_ON) { moFixedMRR += nc.rows[0].monetization.fixedARR / 12; moVarMRR += nc.rows[0].monetization.variableARR / 12; }
        if (CA_ON) {
      ...
      var ivCost = IV_ON ? IV.costAt(a, t, H) : null;
      ...
      months.push({
        ...
        interventions: IV_ON ? { active: rs.active, changes: rs.changes, cost: interventionCost, costItems: ivCost.items, cumulativeCost: cumIv } : null
      });
    }
    ...
    for (var k = 0; k < cohorts.length; k++) {
      ...
      co.finalCustomers = CU_ON ? co.customers : null;
      co.finalPerCustomerRevenue = MO_ON ? MO.revenue(co.money, moR).total : null;
    }
    ...
    return {
      ...
      mechanisms: {
        ...
        customerPhysics: CU_ON,                      /* v2 Gate A */
        monetization: MO_ON,                          /* v2 Gate B */
        genericExpansionBypassed: MO_ON,              /* v2 Gate B: expansionCoefficientAnnual not read */
        cashPhysics: CA_ON,                          /* v2 Gate C: fcf ≠ ebita */
        interventions: IV_ON                         /* v2 Gate D: lawAt(t) differs from the base in some month */
      },
      assumptions: a,
      start: s,
      horizon: H,
      ...
      derived: {
        ...
        impliedAnnualNRR: MO_ON ? null : P * (1 + a.expansionCoefficientAnnual),
        ...
        persistenceAnnualEffective: MO_ON ? null : P,
        persistenceSource: MO_ON ? 'emergent: monetization (contraction reaches variable revenue only)' : CU_ON ? 'derived: L(1 - C)' : 'input',
        expansionSource: MO_ON ? 'emergent: price + usage + adoption (generic coefficient bypassed)' : 'input',
        emergentMonthlyMultiplier: emergentG,
        /* v2 Gate D — the resolved schedule of every hypothesis */
        interventions: IV_ON ? IV.schedule(a, H) : null,
        cash: CA_ON ? {
