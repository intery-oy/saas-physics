...
| Growth decomposition (Customers) | the cumulative economic decomposition of the recurring-revenue movement since M0, on one chart: expansion fills from zero upward, new customers stack on top of it, contraction + churn fills below zero, and a thin ink line is the net that survives. The zero line is the identity — what is above it minus what is below it is the line. Right-edge values give the net, the new contribution, expansion and leakage at the selected month; the legend names the components in full. Contraction and churn are the engine's one leakage flow and are never split |
| Cohort life (Inspect) | clicking a stratum changes the question, so the figure changes with it: the formation canvas and its controls step aside and the same frame carries two charts of ONE cohort — its recurring-revenue life against a quiet dashed line at the balance it was acquired with (the acquisition month marked and ruled, cumulative expansion and leakage printed beneath), and, aligned underneath, cumulative gross profit rising against the dashed acquisition cost with the payback month marked. A bracket at the selected month reads the gap: above/below original, still unrecovered or beyond acquisition cost. Nothing company-level appears — no other strata, no company ARR, no company YoY growth, no Base. The global month still moves, and ‹ Company restores the formation |
