...
(async () => {
  ...
  await pg.waitForTimeout(300);
  const smBounds = await pg.evaluate(() => document.getElementById('causal-slot').innerHTML);
  ...
  await pg.waitForTimeout(300);
  const expBounds = await pg.evaluate(() => document.getElementById('causal-slot').innerHTML);
  ...
  await pg.click('#reset'); await pg.waitForTimeout(300);
  const oneLever = await pg.evaluate(() => document.getElementById('causal-slot').innerHTML.includes('What is driving the delta?'));
  ...
  await pg.waitForTimeout(300);
  const afterOne = await pg.evaluate(() => document.getElementById('causal-slot').innerHTML.includes('What is driving the delta?'));
  rec('attribution panel absent with exactly 1 changed lever', !afterOne, '');
  await pg.evaluate(() => { const i = document.getElementById('f-sm'); i.value = 1300000; i.dispatchEvent(new Event('input')); });
  await pg.waitForTimeout(300);
  const afterTwo = await pg.evaluate(() => document.getElementById('causal-slot').innerHTML.includes('What is driving the delta?'));
